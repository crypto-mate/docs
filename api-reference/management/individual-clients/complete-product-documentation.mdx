...
<Warning>
  **Identity documents must be a photo of the physical document, taken directly with a camera.** Photocopies, scans, screenshots and images exported from an editor are rejected by the KYC review. Upload **JPG or PNG**, up to **10 MB** per image — other formats, including PDF, are not processed reliably.
...
<ParamField body="document" type="object" required>
  ...
  <Expandable title="document">
    ...
    <ParamField body="image_front" type="string" required>
      Base64-encoded front image content. JPG or PNG, up to 10 MB. For identification documents (e.g. `passport`, `national_id`, `drivers_license`, `ssn`) this must be a camera photo of the physical document — see the [KYC image guidelines](/clients/kyc-image-guidelines).
    </ParamField>
