---
title: "Search individual card transactions"
description: "Retrieve transactions for an individual card with filtering, pagination, and sorting."
api: "GET /cards/individual-cards/transactions/{cardId}/search"
---

Searches and retrieves transactions for a specific individual card with filtering, pagination, and sorting. This is the Individual Cards equivalent of the Enterprise [Search card transactions](/api-reference/cards/transactions/search-transactions) endpoint. Requires an API key with access level 1 or higher.
...
<ParamField query="operations" type="array" required>
  Transaction operation types to include. At least one is required. Possible values: `TRANSACTION_AUTHORIZATION`, `TRANSACTION_APPROVED`, `TRANSACTION_REJECTED`, `TRANSACTION_REVERSED`, `TRANSACTION_REFUND`, `WALLET_DEPOSIT`, `OVERRIDE_VIRTUAL_BALANCE`.
</ParamField>
...
<ParamField query="sort_field" type="string">
  Field to sort results by (e.g. `datetime`).
</ParamField>
...
Full parity with the Enterprise endpoint: the pagination envelope and the per-movement fields are identical (both surfaces read from the same transaction source).
...
```bash cURL
curl -X GET "https://api.cryptomate.me/cards/individual-cards/transactions/card_6f1e2a30-4a3c-4b2e-9a1d-5e8c9b7a1f23/search?operations=TRANSACTION_APPROVED&operations=TRANSACTION_REFUND&from_date=2026-01-01&to_date=2026-04-20&size=10&page_number=1&sort_field=datetime&sort_order=DESC" \
