...
Permanently deletes an individual card. The deletion is irreversible: the card stops authorizing transactions and cannot be reactivated. Deleting a card frees one renewable slot of your company's card plan, so you can issue another individual card in its place. Requires an API key with access level 2 or higher.
