...
<ParamField body="metadata" type="object">
  ...
  Some keys are reserved by the platform and you cannot set them here: `wallet_id`, `previous_wallets`, and any key that starts with `mirror_wallet_`. If the metadata carries one of them, the call answers `412` and no card is created. The `message` names the key that was rejected.
</ParamField>
...
{
  "code": "VAL",
  "message": "The metadata key 'wallet_id' is reserved by the platform and cannot be set on card creation."
}
```
