...
<ResponseField name="lifecycle" type="object">
  ...
  <Expandable title="lifecycle">
    ...
    <ResponseField name="state" type="string">
      Whether the clearing added up: how much the issuer settled against how much was authorized. Possible values: `SIMPLE` (a single event, nothing to consolidate), `PARTIAL` (the issuer settled less than authorized, the hold is still in place), `COMPLETE` (settled matches authorized), `OVER_CAPTURE` (the issuer settled more than authorized). Read it together with `outcome`, not instead of it: a purchase whose hold was released settles `0` against `0`, and a fully refunded one settled in full, so both report `COMPLETE`.
    </ResponseField>
