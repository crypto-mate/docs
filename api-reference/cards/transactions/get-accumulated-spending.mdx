...
<Note>
`per_transaction_limit` is not reported here. It caps a single purchase rather than accumulating over a period, so there is no consumed amount to track against it.
</Note>
