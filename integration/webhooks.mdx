...
| Individual card `notification_3ds_authorization_oob` (external authorization channel) | No fixed timeout — the deadline is the `expiry_time` in the event payload, which varies per challenge | None — a decision reported after `expiry_time` finds the challenge already expired |
...
```json
{
  ...
  "data": {
    ...
    "decline_reason": {
      "response_code": "M102",
      "description": "Declined due to insufficient card balance",
      "code": "M102",
      "simple_reason": "Declined due to insufficient card balance",
      "detailed_reason": "Declined due to insufficient card balance"
    },
    "merchant_data": { "name": "Acme Coffee", "mcc_code": "5812" }
  }
}
...
See [Card decline codes](/integration/card-decline-codes) for the full catalog, and for why `decline_reason` can be `null`.
...
#### `notification_3ds_authorization_oob` — Out-of-Band 3DS challenge (Individual Cards)

Delivered when an individual cardholder attempts a purchase that requires an Out-of-Band 3D Secure challenge. CryptoMate never talks to the cardholder directly — your app shows the approve/deny prompt and reports the decision via [Respond to 3DS authorization](/api-reference/cards/individual-cards/respond-3ds-authorization).

This event is sent on the same channel as `authorization`, so acknowledge it with `200 OK` promptly. The challenge itself expires at `expiry_time` — report the cardholder's decision before then, or it is treated as expired.

If CryptoMate cannot deliver this event to your webhook — no webhook configured, an error response, or no response in time — the 3DS authentication is declined and the purchase does not go through. The cardholder is never left waiting on a prompt that never resolves: a delivery failure fails the transaction fast instead of hanging it.

```json
{
  "product": "cards",
  "event_type": "notification_3ds_authorization_oob",
  "operation_id": "25e4988d-9303-4199-983f-e90cc7bf9832",
  "status": "pending",
  "data": {
    ...
    "transaction_amount": "42.50",
    "transaction_currency": "USD",
    "expiry_time": "2025-01-15T12:34:56Z"
  }
}
```
