...
| Individual card `notification_3ds_authorization_oob` (external authorization channel) | Short deadline on the response — acknowledge with `200 OK` promptly | None — a missed acknowledgment is treated as a failed delivery |
...
This event is sent on the same synchronous channel as `authorization`, so acknowledge it with `200 OK` promptly. Unlike `authorization`, your response's `response_code` does not decide the outcome — it only confirms delivery. The challenge itself expires after a short window, so report the cardholder's decision before it does.
