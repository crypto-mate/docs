...
<Warning>
  **Upload a photo of the physical document, taken directly with a camera.** The photo comes straight from a camera and goes straight to the upload — if it passed through a program in between, it will not work. Photocopies, scans, screenshots and images exported from an editor are rejected by the KYC review.

  Upload **JPG or PNG** images, up to **10 MB** each. Other formats are not processed reliably and will fail the review — including PDF and **HEIC / HEIF, which is what an iPhone or a Mac produces by default**.
  ...
  There is no fixed turnaround time for KYC review, and any shortfall in image quality routes the case to manual review — which can take weeks. Read the full [KYC image guidelines](/clients/kyc-image-guidelines) before collecting documents.
</Warning>
