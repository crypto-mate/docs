...
The risk score engine evaluates every purchase in real time. It assigns a numeric **score** by combining up to five signals, each weighted independently. If the score reaches or exceeds a configurable **threshold**, the transaction is declined.
...
<Note>
Signals that require a minimum history return 0 (no contribution) until the card has enough approved transactions. A brand-new card is only evaluated by `decline_rate` and `merchant_country`.
</Note>
...
In this configuration a transaction is rejected only if, for example, high decline rate (0.4), unfamiliar MCC (0.2) and an above-baseline amount (0.2) fire simultaneously (score = 0.8 ≥ 0.8). A single signal is not enough to reach the threshold.
...
Here, high decline rate alone (1.0) exceeds the threshold. Even an unusual hour (0.3) is enough to reject.
