...
A purchase is not a single event. It starts as an **authorization** — money reserved on your balance — and finishes when the card network **settles** it, which normally happens days later, can arrive in several pieces, and occasionally never arrives at all.

Knowing the two phases explains the two things that surprise most integrations: your balance moves before the merchant has actually been paid, and the final amount can differ from the one you approved.

```mermaid
flowchart LR
  A["Cardholder pays"] --> B{"Real-time checks"}
...
The network asks CryptoMate to approve the purchase and expects an answer within about a second. Checks run in this order, and the first one that fails declines the purchase:

1. **Card state** — a blocked or deleted card is declined immediately.
2. **Merchant category and merchant country** — see [Blocked merchant categories](#blocked-merchant-categories) and [Blocked merchant countries](#blocked-merchant-countries).
3. **Velocity rules** — see [Velocity rules](#velocity-rules). A breach declines the purchase and blocks the card.
4. **Balance, limits, risk and your own decision**, evaluated together: the holding balance, the card's [spending limits](#spending-limits), the [risk score engine](#risk-score-engine), and — on cards using `WEBHOOK` — the answer your endpoint returns within 1,200 ms.
...
A declined purchase moves no money and reaches your systems as a `declined` event carrying the reason. See [Card decline codes](/integration/card-decline-codes).

### Stage 2 — Hold

An approved purchase is reserved on your side right away, long before the merchant is paid:

| Balance | Effect at authorization |
|---|---|
| Card's deposit wallet (`TOPUP` cards only) | Debited by the purchase plus fees |
| Program holding wallet | Debited by the purchase plus fees |
| [Fees wallet](#fees-wallet) | Credited with your commission on the purchase |

Your systems receive an `authorized` event. From here the money is no longer spendable, but it has not left the program either — it is a hold, and it stays open until the network settles or releases it.

### Stage 3 — Settlement

Once the merchant submits the purchase for payment, the network settles it and CryptoMate emits a `cleared` event.

The settled amount is frequently **not** the amount you approved. Tips added after the fact, incremental captures (hotels, car rentals, fuel pumps) and exchange-rate re-rating on multi-day settlements all change it. CryptoMate compares what was held against what was settled, recalculates the fees on the settled amount, and adjusts your balance **once**, when the purchase closes:

| Settled vs. authorized | What happens |
|---|---|
| Equal | The purchase closes. Any difference in fees is adjusted. |
| **Less** — a partial settlement | The purchase stays open and more settlements are expected. The remaining hold keeps covering them, so no adjustment is applied yet. |
| **More** — an over-capture | The purchase closes and the difference is debited. |

<Note>
One purchase can settle in several pieces, and each piece emits its own `cleared` event. Reconcile on the transaction, not on the event: only the last one closes the purchase.
</Note>
...
| Dispute won | A dispute resolved in the cardholder's favor credits the disputed amount back. |
| Hold that never settles | If no settlement ever arrives, the hold expires after **32 days** and the reserved funds are released automatically. |

<Warning>
A hold is not a payment. Until it settles or is released, the reserved amount is unavailable for other purchases — it counts against your holding balance and against the card's accumulated limits.
</Warning>
...
sequenceDiagram
  ...
  participant CM as CryptoMate
  participant Y as Your systems
  H->>N: Pays at the merchant
  N->>CM: Authorization request
  CM->>Y: authorization (WEBHOOK cards only)
  Y-->>CM: Approve or decline
  CM-->>N: Decision
  CM->>Y: authorized - funds held
  Note over N,CM: Days later, when the merchant<br/>submits the purchase
  N->>CM: Settlement, in one or more pieces
  CM->>Y: cleared - balance adjusted
```
...
### Funding from another blockchain

If your funds live on **Solana**, **Stellar** or **BNB Chain**, you can ask for an additional deposit address for the holding wallet on that blockchain — a **mirror wallet**. Request one with [Create holding mirror wallet](/api-reference/cards/holding/create-holding-mirror-wallet), and list the ones you already have with [Get holding mirror wallets](/api-reference/cards/holding/get-holding-mirror-wallets). The individual cards program has its own pair — see [Create individual cards holding mirror wallet](/api-reference/cards/individual-cards/create-individual-cards-holding-mirror-wallet).

A mirror wallet is **additional**, not a replacement. The Polygon address stays the program's main deposit address and does not change. You can hold one mirror wallet per blockchain, and a mirror wallet can be added but never removed.

The personal deposit wallets described above work the same way: each card can also have its own Solana, Stellar and BNB Chain mirror addresses — see [Create card mirror wallet](/api-reference/cards/virtual-cards/create-mirror-wallet).

<Warning>
A deposit to a mirror address is **not** credited on the blockchain it landed on. It is transferred to the Polygon holding wallet and credited there, so the balance moves once that transfer settles — not when the deposit confirms on Solana, Stellar or BNB Chain. Expect a delay between the two.

Mirror addresses accept a fixed set of assets that depends on the blockchain — `USDC` on Solana, `USDC` and `USDT0` on Stellar, `USDC` and `USDT` on BNB Chain — and the **10 USD minimum deposit** applies to them as well. The accepted assets come back in the `tokens` field of the mirror wallet endpoints; anything else is not credited. A smaller deposit is neither transferred nor credited: the funds stay at the mirror address and cannot be recovered through the platform.
</Warning>

<Note>
**On BNB Chain the mirror address is the Polygon address you already have.** An EVM address is derived from its key rather than from the network, so the same account exists on both and there is nothing new to create — asking for a BNB Chain mirror wallet only registers it so that deposits there are watched.

Seeing the same `0x…` on two networks is expected. The network still matters: the two balances are independent, and a deposit on BNB Chain is bridged to Polygon rather than appearing there directly.
</Note>
