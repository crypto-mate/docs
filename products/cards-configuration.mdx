---
title: "Virtual Cards — Configuration Guide"
description: "Everything you need to issue, fund, and configure virtual Visa cards for your business."
---
...
| `NONE` | CryptoMate approves transactions automatically against the holding wallet with no external intervention. |
...
For cards using `TOP_UP` approval, each card also has a **personal deposit wallet**. Fetch those addresses with [Get card top-up wallets](/api-reference/cards/virtual-cards/get-card-top-up-wallets). Deposits sent to these wallets are attributed to that specific card and used to settle its purchases.
...
If a `TOP_UP` card's deposit wallet address needs to be rotated (e.g. for compliance or security), use [Rotate deposit wallet](/api-reference/cards/virtual-cards/rotate-deposit-wallet). A new address is generated and associated with the card. The old address stops accepting deposits.
...
Register your webhook URL and key via the Portal (Settings → Webhooks) or via [Subscribe webhook URL](/api-reference/management/company/subscribe-webhook-url).
...
See [Webhooks](/integration/webhooks) for the full envelope format and authentication.
