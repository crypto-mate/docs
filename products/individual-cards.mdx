...
Each customer can hold up to **3 cards** at the same time.

Every card that has not been cancelled counts toward that limit, whatever its status — active, inactive, frozen, expired, or blocked. **Cancelling a card is the only way to free a slot.** Attempting to issue a fourth card is rejected and nothing is created, neither in CryptoMate nor with the card issuer.
